...
lake build Pyspec:static ssz/Ssz:static
leanc -c ffi/shim.c -o .lake/build/shim.o

# The generated library is force-loaded so its exports survive -dead_strip. The
# SSZ library is linked normally: only what Pyspec reaches is pulled in.
leanc -shared -o ".lake/build/libpyspec.$EXTENSION" .lake/build/shim.o \
    -Wl,-force_load,"$PWD/.lake/build/lib/libPyspec_Pyspec.a" \
    "$PWD/.lake/packages/ssz/lean/.lake/build/lib/libssz_Ssz.a" \
    -lleanshared -Wl,-rpath,"$LIBDIR"

echo "built lean/.lake/build/libpyspec.$EXTENSION"
