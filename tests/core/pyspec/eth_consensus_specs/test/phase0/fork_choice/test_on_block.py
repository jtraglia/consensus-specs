...
def test_on_block_before_finalized(spec, state):
    ...
    assert store.finalized_checkpoint.epoch == ge + 2
...
def test_on_block_finalized_skip_slots(spec, state):
    ...
    assert (
        ...
        == spec.get_block_root(state, ge + 1)
        == spec.get_block_root(state, ge + 2)
    )
...
def test_new_finalized_slot_is_justified_checkpoint_ancestor(spec, state):
    ...
    assert state.finalized_checkpoint.epoch == store.finalized_checkpoint.epoch == ge + 2
    assert state.current_justified_checkpoint.epoch == store.justified_checkpoint.epoch == ge + 4
    assert store.justified_checkpoint == state.current_justified_checkpoint

    # Create another chain
    # Forking from epoch 3
    all_blocks = []
    slot = spec.compute_start_slot_at_epoch(ge + 3)
    ...
    assert another_state.finalized_checkpoint.epoch == ge + 3
    assert another_state.current_justified_checkpoint.epoch == ge + 4
...
def test_justification_withholding(spec, state):
    ...
    assert state.finalized_checkpoint.epoch == store.finalized_checkpoint.epoch == ge + 2
    assert state.current_justified_checkpoint.epoch == store.justified_checkpoint.epoch == ge + 3
    assert spec.get_current_epoch(state) == ge + 4
    ...
    assert attacker_state.finalized_checkpoint.epoch == ge + 2
    assert attacker_state.current_justified_checkpoint.epoch == ge + 3
    assert spec.get_current_epoch(attacker_state) == ge + 4
    ...
    assert honest_state.finalized_checkpoint.epoch == store.finalized_checkpoint.epoch == ge + 2
    assert (
        honest_state.current_justified_checkpoint.epoch
        == store.justified_checkpoint.epoch
        == ge + 3
    )
    assert spec.get_current_epoch(honest_state) == ge + 4

    # Create & apply an honest block in epoch 5 that can justify epoch 4
    next_epoch(spec, honest_state)
    assert spec.get_current_epoch(honest_state) == ge + 5
    ...
    assert store.finalized_checkpoint.epoch == ge + 3
    assert store.justified_checkpoint.epoch == ge + 4
    check_head_against_root(spec, store, hash_tree_root(honest_block))
...
def test_justification_update_beginning_of_epoch(spec, state):
    ...
    assert spec.compute_epoch_at_slot(another_state.slot) == ge + 5
    assert another_state.current_justified_checkpoint.epoch == ge + 4
    ...
    on_tick_and_append_step(spec, store, current_time, test_steps)
    assert spec.compute_epoch_at_slot(spec.get_current_slot(store)) == ge + 5
...
def test_incompatible_justification_update_start_of_epoch(spec, state):
    ...
    assert spec.compute_epoch_at_slot(spec.get_current_slot(store)) == ge + 6
    assert state.current_justified_checkpoint.epoch == store.justified_checkpoint.epoch == ge + 5
    assert state.finalized_checkpoint.epoch == store.finalized_checkpoint.epoch == ge + 4
    ...
    assert spec.compute_epoch_at_slot(another_state.slot) == ge + 6
    assert another_state.current_justified_checkpoint.epoch == ge + 3
    assert another_state.finalized_checkpoint.epoch == ge + 2
    _, signed_blocks_temp, another_state = next_epoch_with_attestations(
        spec, another_state, True, False
    )
    signed_blocks += signed_blocks_temp
    assert spec.compute_epoch_at_slot(another_state.slot) == ge + 7
    assert another_state.current_justified_checkpoint.epoch == ge + 6
    assert another_state.finalized_checkpoint.epoch == ge + 2
    ...
    current_time = slot * spec.config.SLOT_DURATION_MS // 1000 + store.genesis_time
    on_tick_and_append_step(spec, store, current_time, test_steps)
    assert spec.compute_epoch_at_slot(spec.get_current_slot(store)) == ge + 8
    ...
    assert store.finalized_checkpoint.epoch == ge + 4
    assert store.justified_checkpoint.epoch == ge + 6
...
def test_justified_update_not_realized_finality(spec, state):
    ...
    assert spec.compute_epoch_at_slot(finalized_block.slot) == ge + 4
...
def test_justified_update_monotonic(spec, state):
    ...
    for signed_block in signed_blocks:
        yield from tick_and_add_block(spec, store, signed_block, test_steps)
    assert spec.compute_epoch_at_slot(spec.get_current_slot(store)) == ge + 7
    assert store.justified_checkpoint.epoch == ge + 6
    assert store.finalized_checkpoint.epoch == ge + 2
    last_block = signed_blocks[-1]
    last_block_root = last_block.message.hash_tree_root()
    ...
    assert spec.compute_epoch_at_slot(spec.get_current_slot(store)) == ge + 7
    assert state.current_justified_checkpoint.epoch == ge + 5
...
def test_pull_up_past_epoch_block(spec, state):
    ...
    assert spec.compute_epoch_at_slot(justifying_slot) == spec.get_current_epoch(state) == ge + 4
...
def test_not_pull_up_current_epoch_block(spec, state):
    ...
    current_time = state.slot * spec.config.SLOT_DURATION_MS // 1000 + store.genesis_time
    on_tick_and_append_step(spec, store, current_time, test_steps)
    assert spec.compute_epoch_at_slot(state.slot) == ge + 5

    # Create a chain within epoch 5 that contains a justification for epoch 5
    signed_blocks, justifying_slot = find_next_justifying_slot(spec, state, True, True)
    assert spec.compute_epoch_at_slot(justifying_slot) == spec.get_current_epoch(state) == ge + 5
    ...
    for signed_block in signed_blocks:
        ...
        check_head_against_root(spec, store, signed_block.message.hash_tree_root())
    assert spec.compute_epoch_at_slot(spec.get_current_slot(store)) == ge + 5
    assert store.justified_checkpoint.epoch == ge + 3
    assert store.finalized_checkpoint.epoch == ge + 2
...
def test_pull_up_on_tick(spec, state):
    ...
    current_time = state.slot * spec.config.SLOT_DURATION_MS // 1000 + store.genesis_time
    on_tick_and_append_step(spec, store, current_time, test_steps)
    assert spec.compute_epoch_at_slot(state.slot) == ge + 6
    assert store.justified_checkpoint.epoch == ge + 5
