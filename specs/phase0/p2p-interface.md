...
def is_within_slot_range(
    ...
    if current_time_ms + MAXIMUM_GOSSIP_CLOCK_DISPARITY < start_time_ms:
        return False
    end_time_ms = compute_time_at_slot_ms(store, slot + slot_range + Slot(1))
...
```python
def compute_subscribed_subnet(node_id: NodeID, epoch: Epoch, index: Uint64) -> SubnetID:
    prefix_bits = compute_attestation_subnet_prefix_bits()
    node_id_prefix = Uint64(node_id >> NodeID(NODE_ID_BITS - prefix_bits))
    node_offset = Uint64(node_id % NodeID(EPOCHS_PER_SUBNET_SUBSCRIPTION))
...
def compute_subscribed_subnets(node_id: NodeID, epoch: Epoch) -> Sequence[SubnetID]:
    return [
        compute_subscribed_subnet(node_id, epoch, Uint64(index))
