...
def test_proposer_boost(spec, state):
    ...
    time = (
        store.genesis_time
        + block.slot * spec.config.SLOT_DURATION_MS // spec.Uint64(1000)
        + late_block_cutoff
        - 1
    )
    ...
    time = store.genesis_time + block.slot * spec.config.SLOT_DURATION_MS // spec.Uint64(1000)
    on_tick_and_append_step(spec, store, time, test_steps)
    yield from add_block(spec, store, signed_block, test_steps)
    assert store.proposer_boost_root == spec.hash_tree_root(block)
    node = get_fork_choice_node(spec, spec.hash_tree_root(block))
...
def test_proposer_boost_is_first_block(spec, state):
    ...
    time = (
        store.genesis_time
        + block_a.slot * spec.config.SLOT_DURATION_MS // spec.Uint64(1000)
        + late_block_cutoff
        - 1
    )
