...
def test_gossip_sync_committee_contribution_and_proof__ignore_superset_contribution(spec, state):
    ...
    signed_superset = spec.SignedContributionAndProof(
        ...
        signature=bls.Sign(privkeys[aggregator_index], spec.compute_signing_root(superset_cap, cap_domain)),
    )
    ...
    result, reason = run_validate_contribution_gossip(
        spec, seen, state, signed_superset, current_time_ms + 500,
    )
    assert result == "valid"
    assert reason is None
    messages.append({"offset_ms": 500, "message": get_filename(signed_superset), "expected": "valid"})
    ...
    signed_subset = create_valid_signed_contribution_and_proof(
        spec, state, aggregator_index, subcommittee_index, subcommittee_pubkeys,
    ...
    result, reason = run_validate_contribution_gossip(
        spec, seen, state, signed_subset, current_time_ms + 600,
    )
    assert result == "ignore"
    assert reason == "already seen contribution for this data"
    messages.append({
        "offset_ms": 600, "message": get_filename(signed_subset),
        "expected": "ignore", "reason": reason,
    })
...
def test_gossip_sync_committee_contribution_and_proof__valid_non_superset_contribution(spec, state):
    ...
    result, reason = run_validate_contribution_gossip(
        spec, seen, state, signed_subset, current_time_ms + 500,
    )
    assert result == "valid"
    assert reason is None
    ...
    # Use second_validator_index as the aggregator (works in minimal preset where all are aggregators)
    domain = spec.get_domain(state, spec.DOMAIN_SYNC_COMMITTEE, epoch)
    signing_root = spec.compute_signing_root(block_root, domain)
    sig1 = bls.Sign(privkeys[aggregator_index], signing_root)
    sig2 = bls.Sign(privkeys[second_validator_index], signing_root)
    ...
    selection_proof = spec.get_sync_committee_selection_proof(
        state, state.slot, subcommittee_index, privkeys[second_validator_index],
    )
    superset_cap = spec.ContributionAndProof(
        aggregator_index=second_validator_index,
        contribution=superset_contribution,
        selection_proof=selection_proof,
    )
    cap_domain = spec.get_domain(state, spec.DOMAIN_CONTRIBUTION_AND_PROOF, epoch)
    signed_superset = spec.SignedContributionAndProof(
        message=superset_cap,
        signature=bls.Sign(
            privkeys[second_validator_index], spec.compute_signing_root(superset_cap, cap_domain),
        ),
    )
    ...
    result, reason = run_validate_contribution_gossip(
        spec, seen, state, signed_superset, current_time_ms + 600,
    )
    assert result == "valid"
    assert reason is None
    messages.append({"offset_ms": 600, "message": get_filename(signed_superset), "expected": "valid"})
...
def test_gossip_sync_committee_contribution_and_proof__ignore_duplicate_aggregator(spec, state):
    ...
    result, reason = run_validate_contribution_gossip(
        spec, seen, state, signed_cap1, current_time_ms + 500,
    )
    assert result == "valid"
    assert reason is None
    ...
    result, reason = run_validate_contribution_gossip(
        spec, seen, state, signed_cap2, current_time_ms + 600,
    )
    assert result == "ignore"
    assert reason == "already seen contribution from this aggregator"
    messages.append({
        "offset_ms": 600, "message": get_filename(signed_cap2),
        "expected": "ignore", "reason": reason,
    })
...
def test_gossip_sync_committee_contribution_and_proof__reject_invalid_selection_proof(spec, state):
    ...
    # Replace selection proof with one signed by a different key
    wrong_key = privkeys[(aggregator_index + 1) % len(privkeys)]
    signed_cap.message.selection_proof = spec.get_sync_committee_selection_proof(
        state,
        state.slot,
        subcommittee_index,
        wrong_key,
