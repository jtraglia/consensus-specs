...
```python
def bytes_to_bls_field(b: Bytes32) -> int:
    """
...
```python
def bls_field_to_bytes(x: int) -> Bytes32:
...
```python
def blob_to_polynomial(blob: Blob) -> Sequence[int]:
    """
...
```python
def compute_challenge(blob: Blob, commitment: KZGCommitment) -> int:
    """
...
```python
def g1_lincomb(points: Sequence[KZGCommitment], scalars: Sequence[int]) -> KZGCommitment:
    """
...
```python
def compute_powers(x: int, n: uint64) -> Sequence[int]:
    """
...
```python
def compute_roots_of_unity(order: uint64) -> Sequence[int]:
    """
    Return roots of unity of ``order``.
    """
    assert (BLS_MODULUS - 1) % int(order) == 0
    root_of_unity = pow(PRIMITIVE_ROOT_OF_UNITY, (BLS_MODULUS - 1) // int(order), BLS_MODULUS)
...
```python
def evaluate_polynomial_in_evaluation_form(polynomial: Sequence[int], z: int) -> int:
    """
    ...
    result = result * (BLS_MODULUS + pow(z, width, BLS_MODULUS) - 1) * inverse_width
...
def verify_kzg_proof_batch(commitments: Sequence[KZGCommitment],
    ...
    proof_z_lincomb = g1_lincomb(
        proofs,
        [((z * r_power) % BLS_MODULUS) for z, r_power in zip(zs, r_powers)],
    )
...
```python
def compute_quotient_eval_within_domain(z: int, polynomial: Sequence[int], y: int) -> int:
    """
    ...
    for i, omega_i in enumerate(roots_of_unity_brp):
        ...
        f_i = BLS_MODULUS + polynomial[i] - y % BLS_MODULUS
...
```python
def compute_kzg_proof_impl(polynomial: Sequence[int], z: int) -> Tuple[KZGProof, int]:
    """
    ...
    denominator_poly = [(x - z) % BLS_MODULUS for x in roots_of_unity_brp]
