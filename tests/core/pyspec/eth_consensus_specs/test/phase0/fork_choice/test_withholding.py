...
def test_withholding_attack(spec, state):
    ...
    # Create the attack block that includes justifying attestations for epoch ge+4
    # This block is withheld & revealed only in epoch ge+5
    ...
    # Create an honest chain in epoch ge+5
    next_epoch(spec, state)
...
def test_withholding_attack_unviable_honest_chain(spec, state):
    ...
    # Create the attack block that includes justifying attestations for epoch ge+5
    # This block is withheld & revealed only in epoch ge+6
    signed_blocks, justifying_slot = find_next_justifying_slot(spec, state, True, False)
    assert spec.compute_epoch_at_slot(justifying_slot) == spec.get_current_epoch(state)
    assert len(signed_blocks) > 1
    signed_attack_block = signed_blocks[-1]
    ...
    # Create an honest chain in epoch ge+6
    next_epoch(spec, state)
    ...
    assert current_epoch == ge + 6
    # assert store.voting_source[honest_block_root].epoch == ge + 3
