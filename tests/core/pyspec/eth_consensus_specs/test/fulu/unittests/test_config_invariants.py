...
def test_invariants(spec):
    assert spec.FIELD_ELEMENTS_PER_BLOB % spec.Uint64(spec.FIELD_ELEMENTS_PER_CELL) == 0
    assert spec.FIELD_ELEMENTS_PER_EXT_BLOB % int(spec.NUMBER_OF_COLUMNS) == 0
    ...
    assert spec.NUMBER_OF_COLUMNS % spec.Uint64(spec.config.DATA_COLUMN_SIDECAR_SUBNET_COUNT) == 0
...
def test_polynomial_commitments_sampling(spec):
    assert 2 * int(spec.FIELD_ELEMENTS_PER_BLOB) == spec.FIELD_ELEMENTS_PER_EXT_BLOB
